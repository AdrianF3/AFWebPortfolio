  ...
  return (<>
    <section className='px-4 text-center'>            
        ...
        <div className='text-xl font-medium underline underline-offset-2 px-4'>
            <h3>{props.currentDetails.name}'s Details</h3>
        </div>
        {/* Table Details Section */}
        <div className='flex flex-col'>
            <p>Total Paid:  {moneyFormatter.format(props.currentDetails.totalPaid)}</p> 
            ...
            {props.currentDetails.owes.details.length > 0 ? props.currentDetails.owes.details.map((roommate, index) => {                
                ...
                return <p>{roommate.name}: {moneyFormatter.format(roommate.amount)}</p>
