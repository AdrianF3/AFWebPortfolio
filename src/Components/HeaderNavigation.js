import React, {useState} from 'react'
import {NavLink} from 'react-router-dom'

export default function HeaderNavigation() {
    const [isMobileMenuOpen,
        setIsMobileMenuOpen] = useState(false)

    const defaultMenu = (
        <div className='hidden md:flex flex-row gap-4'>
            <NavLink to='/'>
                <p className=''>Home</p>
            </NavLink>
            <NavLink to='/weather'>
                <p className=''>Weather Project</p>
            </NavLink>
            <NavLink to='ytcontroller'>
                <p>YouTube Controller</p>
            </NavLink>
            <NavLink to=''>
                <p>Ector Grow</p>
            </NavLink>
            <NavLink to=''>
                <p>Get In Touch</p>
            </NavLink>
        </div>
    )

    const mobileMenu = (
        <ul className="md:flex px-4 mx-auto font-semibold font-heading space-x-12">
            <li>
                <a className="hover:text-gray-200" href="#">HomeMM</a>
            </li>
            <li>
                <a className="hover:text-gray-200" href="#">Portfolio ProjectsMM</a>
            </li>
            <li>
                <a className="hover:text-gray-200" href="#">Ector GrowMM</a>
            </li>
            <li>
                <a className="hover:text-gray-200" href="#">Get In TouchMM</a>
            </li>
        </ul>
    )

    // https://tailwindcomponents.com/component/navbar-hamburger-menu-for-ecommerce
    console.log('isMobileMenuOpen', isMobileMenuOpen)

    return ( <> <div className="flex flex-wrap place-items-center h-fit">
        <section className="relative mx-auto">
            {/* <!-- navbar --> */}
            <nav className="flex justify-between bg-gray-900 text-white w-screen">
                <div className="py-6 flex justify-between w-full">
                    <a className="text-3xl font-bold font-heading px-5" href="#">
                        {/* <!-- <img class="h-9" src="logo.png" alt="logo"> --> */}
                        Adrian Fregoso
                    </a>

                    {/* <!-- Nav Links --> */}
                    {/* Full Display */}
                    <div className='px-5'>
                        {isMobileMenuOpen
                            ? mobileMenu
                            : defaultMenu}
                    </div>

                </div>
                {/* <!-- Open/Close Menu --> */}
                <div
                    className=' md:hidden'
                    onClick={() => {
                    isMobileMenuOpen
                        ? setIsMobileMenuOpen(false)
                        : setIsMobileMenuOpen(true)
                }}>
                    <i className='material-icons'>menu</i>
                </div>
            </nav>

        </section>
    </div> </>
  )
}